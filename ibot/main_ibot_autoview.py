...
from loader import ImageFolderMaskAutoView
...
def get_args_parser():
    ...
    # augmentation search
    parser.add_argument("--arch_lr", default=6e-5, type=float, help="""search lr""")
    parser.add_argument("--tp_lr", default=1e-5, type=float, help="""search lr""")
    return parser

def step_scheduler(init_lr, step_size, epoch, min_lr=6e-6):
    step_lrs = []
    for _ in range(epoch):
        if init_lr < min_lr:
            step_lrs.append(min_lr)
        else:
            step_lrs.append(init_lr)
            init_lr = init_lr*step_size
    return step_lrs
...
def train_ibot(args):
    ...
    dataset = ImageFolderMaskAutoView(
        args.data_path, 
        transform=None,
    ...
    ops_weights = Parameter(1e-3 * torch.ones(len(data_loader.dataset.policy_lists)).cuda(), requires_grad=True)

    init_tp = 0.7
    tp_alpha = np.log(init_tp / (1 - init_tp))
    tp_param = Parameter((torch.ones(1, requires_grad=True) * tp_alpha).cuda(), requires_grad=True)

    aug_optimizer = torch.optim.Adam([ops_weights], lr=args.arch_lr, betas=(0.5, 0.999), weight_decay=0.0)
    aug_optimizer_scheduler = step_scheduler(6e-5, 0.8, args.epochs, 6e-6)

    optimizer_tp = torch.optim.Adam([tp_param], lr=args.tp_lr, betas=(0.5, 0.999))
    for i, param_group in enumerate(optimizer_tp.param_groups):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, param_group in enumerate(aug_optimizer.param_groups):
            param_group["lr"] = -aug_optimizer_scheduler[epoch]  # 负的lr
        train_stats = train_one_epoch(student, teacher, teacher_without_ddp, ibot_loss,
            data_loader, optimizer, aug_optimizer, optimizer_tp, ops_weights,tp_param, lr_schedule, wd_schedule, momentum_schedule,
        ...
        save_dict = {
            ...
            'optimizer_tp': optimizer_tp.state_dict()
        }
        ...
        op_soft_prob = (ops_weights / 1.0).softmax(dim=-1).clone().detach().cpu()
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                     'epoch': epoch,
                     'op_soft_prob': op_soft_prob.tolist(),'opp': tp_param.sigmoid().clone().detach().cpu().numpy().tolist()}
...
import warnings
def gumbel_softmax_rets(logits, index, tau=1, hard=True, eps=1e-10, dim=-1):
    if eps != 1e-10:
    ...
    y_hard = torch.zeros(logits.size()).scatter_(dim, index, 1.0)
    ret = y_hard[index] - y_soft[index].detach() + y_soft[index]
    return ret

def image_rets(origin_images, ops_weights, batch_indexs, tp_param, ori_image):
    crop_rets = []
    for crop_num in range(len(origin_images)):
        if crop_num > 1:
            break
        batch_rets = []
        for batch, indexs in enumerate(batch_indexs[crop_num]):  # 64x2
            rets = gumbel_softmax_rets(ops_weights, indexs)
            batch_rets.append(rets)
        crop_rets.append(batch_rets)

    for crop_num, crop_ret in enumerate(crop_rets):
        image_size = origin_images[crop_num].shape
        # ret1 = torch.ones(image_size[0], 1).cuda()
        # for i, rets in enumerate(crop_ret):
        #     ret1[i] = rets
        ret1 = torch.stack(crop_ret, 0).unsqueeze(1).cuda()
        origin_images[crop_num] = ((origin_images[crop_num].view(image_size[0], -1)) * ret1).view(image_size)
        op2_prob = tp_param.sigmoid()
        mix_crop = op2_prob * origin_images[crop_num] + (1 - op2_prob) * ori_image
        origin_images[crop_num] = (origin_images[crop_num] - mix_crop).detach() + mix_crop

    return origin_images

def train_one_epoch(student, teacher, teacher_without_ddp, ibot_loss, data_loader,
                    optimizer, aug_optimizer, optimizer_tp, ops_weights,tp_param, lr_schedule, wd_schedule, momentum_schedule,epoch,
                    fp16_scaler, args):
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Epoch: [{}/{}]'.format(epoch, args.epochs)

    print((ops_weights / 1.0).softmax(dim=-1))
    print(tp_param.sigmoid())
    aug_optimizer.zero_grad()
    optimizer_tp.zero_grad()
    data_loader.dataset.ops_weights = ops_weights.cpu()
    data_loader.dataset.tp_param = tp_param.cpu()

    real_labels, pred_labels = [], []
    for it, (images, labels, masks, batch_indexs, ori_image) in enumerate(metric_logger.log_every(data_loader, 100, header)):
        ...
        ori_image = ori_image.cuda(non_blocking=True)
        images = image_rets(images, ops_weights, batch_indexs, tp_param, ori_image)
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            with torch.no_grad():
                teacher_output_ori = teacher(ori_image)[0]
            ...
            all_loss, view_reg_loss = ibot_loss(student_output, teacher_output, teacher_output_ori, student_local_cls, masks, epoch)
            loss = all_loss.pop('loss') + view_reg_loss
        ...
        if fp16_scaler is None:
            ...
            if args.clip_grad:
                ...
                fp16_scaler.unscale_(aug_optimizer)
                fp16_scaler.unscale_(optimizer_tp)
                param_norms = utils.clip_gradients(student, args.clip_grad)
            utils.cancel_gradients_last_layer(epoch, student,
                                              args.freeze_last_layer)
            fp16_scaler.step(optimizer)
            fp16_scaler.step(aug_optimizer)
            fp16_scaler.step(optimizer_tp)
            fp16_scaler.update()
        ...
        metric_logger.update(view_reg_loss=view_reg_loss.item())
...
class iBOTLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, teacher_output_ori, student_local_cls, student_mask, epoch):
        """
        ...
        # 注意log_softmax
        teacher_cls_c = F.softmax((teacher_cls - self.center) / temp, dim=-1)
        teacher_cls_c = teacher_cls_c.chunk(self.ngcrops)
        teacher_output_ori = (teacher_output_ori - self.center) / temp
        view_reg_loss = torch.sum(-teacher_cls_c[0] * F.log_softmax(teacher_output_ori, dim=-1), dim=-1).mean() + \
                       torch.sum(-teacher_cls_c[1] * F.log_softmax(teacher_output_ori, dim=-1), dim=-1).mean()
        view_reg_loss /= 2

        self.update_center(teacher_cls, teacher_patch)                  
        return total_loss, -view_reg_loss
